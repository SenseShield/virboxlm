...
type
  TBTMemoryModule = Pointer;


{$IF CompilerVersion < 14.5}
  // Delphi6(Borland Delphi Version 14.0) 不支持 UInt64类型，特殊兼容处理
  UInt64 = LongWord;
  {$EXTERNALSYM UInt64}
{$IFEND}
...
function BTMemoryLoadLibary(fp_data: Pointer; const f_size: int64;
  ...
  f_DllProcessAttach: Boolean = True): TBTMemoryModule;

// return value is nil if function fails
function BTMemoryGetProcAddress(fp_module: TBTMemoryModule;
  const fp_name: PChar): Pointer;
// free module
procedure BTMemoryFreeLibrary(fp_module: TBTMemoryModule);
// returns last error
...
type
  PBTMemoryModule = ^BT_MEMORY_MODULE;
  ...
  begin
    ...
    begin
      l_handle := LoadLibrary(PAnsiCharToPChar(IncF(fp_module^.codeBase,
            lp_importDesc^.Name)));
      if ((l_handle = INVALID_HANDLE_VALUE) or (l_handle = 0)) then
      begin
        lastErrStr :=
...
begin
  Result := nil;
  l_idx := -1;
  lp_module := PBTMemoryModule(fp_module);
  l_directory := GetHeaderDictionary(lp_module, IMAGE_DIRECTORY_ENTRY_EXPORT);
  if l_directory^.Size = 0 then
  begin
    lastErrStr := 'BTMemoryGetProcAddress: no export table found';
    exit;
  end;
  l_exports := IncF(lp_module^.codeBase, l_directory^.VirtualAddress);
  if ((l_exports^.NumberOfNames = 0) and (l_exports^.NumberOfFunctions = 0)) then
  begin
    lastErrStr := 'BTMemoryGetProcAddress: DLL doesn''t export anything';
    exit;
  end;
  // search function name in list of exported names
  l_nameRef := IncF(lp_module^.codeBase, l_exports^.AddressOfNames);
  l_ordinal := IncF(lp_module^.codeBase, l_exports^.ADDressOfNameOrdinals);
  l_hiname :=  ((Integer(fp_name) shr 16) and $ffff);
  if ( l_hiname = 0) then
  begin
    l_idx := (Integer(fp_name) and $ffff) - l_exports^.Base;
  end
  else
  begin
    for l_i := 0 to l_exports^.NumberOfNames do
    begin
      if StrComp(fp_name, PAnsiCharToPChar(IncF(lp_module^.codeBase, l_nameRef^))
        ) = 0 then
      begin
        l_idx := l_ordinal^;
        break;
  ...
  Result := IncF(lp_module^.codeBase,
    PUInt64(IncF(lp_module^.codeBase, IncF(l_exports^.AddressOfFunctions,
          l_idx * 4)))^);
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if UInt64(lp_module^.modules^) <> INVALID_HANDLE_VALUE then
          FreeLibrary(UInt64(lp_module^.modules^));
      ...
      HeapFree(GetProcessHeap(), 0, lp_module);
      Pointer(lp_module) := nil;
    end;
  end;
end;
