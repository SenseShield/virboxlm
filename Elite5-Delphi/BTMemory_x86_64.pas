...
begin
  ...
  // DLL doesn't export anything    modify by lihp
  //if (exportDir.NumberOfNames = 0) or (exportDir.NumberOfFunctions = 0) then
  if (exportDir.NumberOfNames = 0) and (exportDir.NumberOfFunctions = 0) then
  begin
    SetLastError(ERROR_PROC_NOT_FOUND);
    Exit;
  end;
  ...
  hiname :=  ((Integer(name) shr 16) and $ffff);
  idx := -1;
  if ( hiname = 0) then
  begin
    idx := (Integer(name) and $ffff) - exportDir^.Base;
  end
  else
  begin
